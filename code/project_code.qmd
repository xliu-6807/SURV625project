...
pacman::p_load(readxl, knitr, sampling, tidyverse)
...
MI_school_samples |>  select(Outcome, expect_mean, s_sqrd, roh, 
...
-   Using the proportionate allocation by strata computed earlier, we assign and add cluster allocation by stratum.
...
strata_Prop_allocate <- strata_Prop_allocate |> 
  # Ensure at least 1 cluster per
  mutate(clusters_allocated = pmax(1, round(n_opt * prop_allocation)))         
...
-   We also calculate the first stage probability of selection using the equation below, which we take the sum of MOS in each strata multiplied by the number of schools to be selected, divided by the total MOS.

$$
\frac{nMOS_i}{\sum_t MOS_i}
$$
...
school_frame <- school_frame |> 
  # compute 1st stage
  mutate( pik =  pmin(1, ( (n_opt * tot_all) / sum(tot_all) )), 
          selected  = UPsystematic(pik))

sampled_schools <- school_frame |> 
  filter(selected == 1) |> arrange(tot_all)

sampled_schools |> 
  select(Region, BNAME, tot_all) |>
  rename(MOS = tot_all) |> 
  arrange(Region) |> 
  kable()
...
-   We sort selected schools by MOS size and collapse neighboring zones to create pseudo strata that have more than one selection. We print the first one here as an example.

```{r}

num_pairs <- floor(nrow(sampled_schools) / 2)

# create sampled pairs
paired_strata <- sampled_schools |> 
  mutate(group_id = rep(1:num_pairs, each = 2,
                        length.out = nrow(sampled_schools))) |> 
  group_by(group_id) |> 
  group_split()
...
BELOW CANNOT BE RIGHT, WE GET THE SAME M SIZE FOR EACH STRATA

```{r}
# overall sampling fraction
f <- samp_frac # calculated earlier .0057

school_frame |> 
  group_by(Region) |> 
  reframe(schools = n(), students = sum(tot_all)) |> 
  # compute sampling rates
  mutate(f_h = f * (N / (n_opt * students)),
         # compute subsample size
         n_students =   f_h * students) |> 
  kable()
```
...
2.  Do each of the schools that you sampled in a given region have the minimum sufficient size, given the stratum-specific subsample sizes computed in Task #1? Do subsequent schools on the list have the minimum sufficient size? If not, what will you do?
    ...
    2.  School 07809 does not meet the requirement of 54 students. The next school in sequence does not meet the minimum requirement with only 29 student, but the next school in sequence does meet the requirement. We will need to link these two schools.
...
3.  Begin to describe how you will physically select the subsample of students within a given sampled school (or set of linked schools). What will your second-stage sampling rate be for a given school within a given stratum? How will you acquire the updated rosters from each school? What technique will you use to select the sample at the specified second-stage rate?
    ...
    2.  The linking is done by first selecting $n_{opt}$ schools from each strata. When the next school on the list do not meet the sufficient size required of $m_{opt}$ student size we move down the list until the first school that meets the minimum requirement is achieved. For all the schools that did not meet the requirement they are cumulated backwards until a linked school of minimum sufficient size is created. We do this process for all the schools initially selected.
