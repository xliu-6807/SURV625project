...
MI_school_samples <- MI_school_samples |> 
  ...
  mutate(desired_deff = c(2.5, 2.0, 1.7),
         # compute roh
         roh = (desired_deff - 1) / (m - 1)
         )

MI_school_samples |> select(Outcome, desired_deff, roh) |>  kable()
```
...
-   Here we compute the total sample size under `total_nm` and projected cost which is right at budget.
...
-   We also compute the total sample size for these three options, and from here we can compute the total cost as $n \times  c_n + n \times m \times c_m$ which we use to determine the optimum subsample size for each variable.
    -   option 1 which is 96 clusters with size 43 and total sample of 4,207.
...
MI_school_samples |> 
  ...
  mutate(m_opt = m_opt[1], # optimum m from first row
         n_opt = n_opt[1],
         total_nm = m_opt*n_opt,
         deff_new = 1 + (m_opt-1) * roh) |> 
  mutate(projected_cost = (c_n * n_opt) + (c_m * n_opt * m_opt),
...
-   Option 2, which is 87 clusters with size 53 and total sample of 4,221.
...
-   Option 3, which is 80 clusters with size 64 and total sample of 5,174.

```{r}
MI_school_samples |> 
  select(Outcome, roh, m_opt, n_opt, total_nm) |> 
  # we can print projected total cost for n=50
  mutate(m_opt = m_opt[3], # optimum m from first row
         n_opt = n_opt[3],
         total_nm = m_opt*n_opt,
         deff_new = 1 + (m_opt-1) * roh) |> 
  mutate(projected_cost = (c_n * n_opt) + (c_m * n_opt * m_opt),
...
-   Option 3 gives us the largest sample size (5,174) with highest design effects (see option 3) for 2 of 3 outcome variables. This is the optimal design while being at budget.
...
-   We will need to use the optimum n and m of each row and project them to the other rows to calculate the design effect: deff = (SAM_variance / SRS_variance). This should give us an idea of the mean deff for three total projections.

```{r}
# compute deff under the optimum 
#MI_school_samples <- 

MI_school_samples

MI_school_samples |> 
  mutate(var_srs =  (1-( (n_opt*m_opt) / (m*n)) ) * 
           ( (expect_mean*(1-expect_mean)) / (total_nm - 1))  ) |> 
  select(Outcome, deff_new, var_srs)

#n_new <- 40
#nm_new <- n_new * m_new
#f_new <- nm_new / NM
#s_sqrd_new <- ( sum(y_bar^2) - ( ((sum(y_bar))^2 / n_new)) / ((m^2)*(n_new-1)) )
#var_srs_new <- (1 - f_new) * (1 / (nm_new^2)) * s_sqrd_new; var_srs_new
```
